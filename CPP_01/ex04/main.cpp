...
/*   Updated: 2024/04/22 17:10:28 by pveiga-c         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
bool open_copy_name(std::string fileName, std::ifstream &file_original, std::ofstream &file_replace)
{
	...
	if(!file_original)
	{
		std::cerr << "Unable to open the file origin_file";
		std::cout << std::endl;
		return (0);
	}
...
int main(int argc , char **argv)
{
	...
	if(argc != 4)
	{
		...
		std ::cout << "Usage: ./sedIsForLosers file_original string_to_replace string_to_replace_with" << std::endl;
		return (0);
	}
